/*
	...
	* 파일 이름	: Ex_02
	* 설명		: 태양빛 도달 시간 계산 프로그램 ver 1.2
	* 작성자	: 박광렬 
	* 제작 시각	: 2019.04.03		12.29
	* 수정 시각 : 
...
int main(void) {
	...
	double Distance = 1.496e8;	//거리 값을 선언한 후 지수 형태로 초기화합니다. 	double
	
	double time;	//계산된 시간 값을 저장할 변수를 선언합니다. 	double
	time = Distance / Light_Speed;	//시간 값을 계산합니다. 
	
	int minute = time / 60;	//시간 값 중 분 값을 계산 
	double second = time - 60 * minute;	//시간 값 중 초 값을 계산 
		
	printf("빛의 속도는 %lfkm/s \n", Light_Speed);	//빛의 속도 값을 출력합니다. 
	printf("태양과 지구와의 거리 %lfkm \n\n\n", Distance);	//거리 값을 출력합니다. 
	printf("따라서 도달 시간은 %lf초이고 이는 %d분 %lf초입니다.\n", time, minute, second);	//시간 값, 분, 초 값읋 출력합니다. 
